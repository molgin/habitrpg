...
function($rootScope, User, $timeout, $state) {
  ...
  var tour = new Tour({
    backdrop: true,
    template: '<div class="popover" role="tooltip"> <div class="arrow"></div> <h3 class="popover-title"></h3> <div class="popover-content"></div> <div class="popover-navigation"> <div class="btn-group"> <button class="btn btn-sm btn-default" data-role="prev">&laquo; Prev</button> <button class="btn btn-sm btn-default" data-role="next">Next &raquo;</button> <button class="btn btn-sm btn-default" data-role="pause-resume" data-pause-text="Pause" data-resume-text="Resume">Pause</button> </div> '+ /* +'<button class="btn btn-sm btn-default" data-role="end">' + window.env.t('ok') + '</button> '+ */ +' </div> </div>',
    //onEnd: function(){
  ...
  var goto = function(chapter, page, force){
    var isNext = page == (tour.getCurrentStep()+1);
    if ( (!isNext || User.user.flags.tour[chapter] > page) && !force ) return;
    var updates = {};updates['flags.tour.'+chapter] = page;
    User.set(updates); //fixme
  ...
  var watcher = $rootScope.$watch('User.user.ops.update', function(updateFn){
    ...
    goto('intro', User.user.flags.tour.intro || 0, true);
